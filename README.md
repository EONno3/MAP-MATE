...
- **단축키/헬프 UX 개선**: 도움말 모달에서 단축키/조작법을 한눈에 확인하고, 주요 동작을 더 일관되게 안내합니다.
- **룸 에디터 네비게이터 개선**: `Room Navigator` 가시성/조작감(줌/패닝/툴팁) 개선 및 관련 유틸/테스트 추가
- **저장/이탈 안전장치 강화**: 저장 단축키 훅(`useSaveHotkey`) 및 저장되지 않은 변경사항 경고 다이얼로그 추가
- **튜토리얼/번역 데이터 정리 및 테스트 보강**: 튜토리얼 번역/가이드 문구 정리, 커버리지 성격의 테스트 추가
- **Unity Importer 자동화/안정화**: 자동 세팅/리임포트 플로우 개선 및 EditMode 테스트 보강
- **개발 환경 안정화**: `.gitattributes`로 줄바꿈(LF/CRLF) 정규화 → 불필요한 diff churn 감소

## 🌟 v1.6 업데이트 주요 내역
- **브라우저 로컬 저장소(Local Save) 기능 추가**: `Ctrl + S` 단축키를 통한 빠른 저장 및 여러 맵 슬롯 관리 지원
...
> 로컬 개발(Vite dev server)은 기본 포트가 `3000`입니다. (`packages/frontend/vite.config.ts` 참고)
